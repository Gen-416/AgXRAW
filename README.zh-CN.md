...
旋钮（ρ 基准、白点留量、高光压缩起点）留给用户，默认即数学口径。技术细节见
...
- **观察（observe，默认）**：`--film portra400`。胶片只声明"观察者看见了什么"——
  ...
  取向的风格配对（感色强度倍率与 AgX 色彩浓淡，界面可见可改）。颜色仍由 AgX
...
- **接管（full，默认）**：`--film portra400 --film-mode full`。颜色的形成本身被
  ...
  印相曝光方式 → 相纸显影 → B2 观看 → 灰阶校色 → 参考印相外观配方（默认 @1.0）。
  默认状态按数据手册标定，所以同样克制：与观察相比 ΔE 2.8 / 4.0 / 2.6，
  可见像素 5% / 30% / 16%，Velvia 的绿系色相转了约 8°。
- **完整冲印 · 调色强度 ×3 + 层间效应 β 1.5**：同一条接管链，把链上两个明确标注为编辑取向的层
...
"胶片味"的上限，而是测量底座；外观配方与层间效应是链上明确标注为编辑取向的
...
是光谱底座加 modelled 层间效应——公开光谱数据（乳剂感度、染料密度、相纸、光源）
约束下的三刺激重建，加声明的 modelled 层间项，关闭编辑性外观，灰阶全程中性。它不是
...
曝光（B1）→ 印相曝光方式（τ）→ 相纸显影曲线 → 观看链（B2）。印相材料、印相曝光方式
（固定 / 随胶片曝光补偿 / 自定义色头+印相曝光；随曝光补偿对全部负片可用，反转片没有
印相环节一律固定）、灰阶校色、自定义冲洗、高光预压缩、颗粒与光晕
...
白墙与受光的卵石地纹丝不动；全图亮度差中位数为 0。左边是全程中性（灰阶
严格保持中性，`--film-neutralization technical-neutral`，CLI 默认；GUI 默认"跟随
成片调色"，由所选的成片调色决定灰阶怎样校正），右边是数据手册原样（`native`）。
...
- **观察并复现实验**：本地图形界面和命令行使用同一套设置，图形界面覆盖命令行全部
  可调项（报告/诊断输出除外）。图形界面的"RAW 过曝标记"开关
  把 RAW 里达到满阱 97% 以上的像素（高光降饱和区，未必已溢出）按 CFA 通道标在预览上，并列出全尺寸完全过曝百分比；命令行默认只打印写出的文件，完整分析
...
预览卡上的"RAW 过曝标记"开关把 RAW 里达到满阱 97% 以上的像素（渲染里高光降饱和生效的区域，未必已经溢出）按通道标在预览图上，旁边另列全尺寸的完全过曝百分比——只有这个数字才证明信息丢失（红/绿/蓝＝该通道
...
# 使用胶片型号（默认 observe：胶片声明观察者，AgX 显影）
...
（halation、capture bloom、介质柔化、颗粒采样）与胶片核逐像素链（层曝光/色度场、
特性曲线、层间效应、四面体 LUT、高光预压缩、cast 除法）；渲染方案和出错时的回退处理仍由
...
`pip install .` 也会编译内核；没有 Rust 工具链（或设 `DNGSCAN_BUILD_NATIVE=OFF`）时
安装纯 Python 包。
