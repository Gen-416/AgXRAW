...
- **观察并复现实验**：本地图形界面和命令行使用同一套设置。图形界面的"RAW 满阱"开关
  把 RAW 里达到满阱 97% 以上的像素（已溢出或即将溢出）按 CFA 通道标在预览上，并列出全尺寸硬剪切百分比；命令行默认只打印写出的文件，完整分析
...
预览卡上的"RAW 满阱"开关把 RAW 里达到满阱 97% 以上的像素（已溢出或即将溢出，也就是渲染里色度退让生效的区域）按通道标在预览图上，旁边另列全尺寸的硬剪切百分比（红/绿/蓝＝该通道
