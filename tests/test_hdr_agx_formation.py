...
def _coreimage_available() -> bool:
    try:
        ...
        return coreimage_decode.runtime_available(interactive=True)
    except Exception:
        return False
...
class Raw9HdrCouplingTests(unittest.TestCase):
    ...
    masks, independent RAW reference evidence. These gates pin the couplings that keep its HDR
    output honest — capped chroma freedom, separately measured reliable tail, mask-free
    ...
    def test_independent_reference_preserves_valid_daylight_headroom(self) -> None:
        libraw_tail = self.pairs["libraw"][3].tone.reliable_tail_ev
        bundle, analysis, _, hdr = self.pairs["coreimage"]
        self.assertEqual(hdr.reliability_source, "sensor-reference")
        self.assertIsNotNone(bundle.scene_reliable_reference_rec2020)
        self.assertGreater(len(bundle.scene_reliable_reference_rec2020), 256)
        self.assertGreater(hdr.tone.rendered_headroom_ev, 0.0)
        self.assertTrue(math.isfinite(libraw_tail) and math.isfinite(hdr.tone.reliable_tail_ev))
        self.assertLess(abs(hdr.tone.reliable_tail_ev - libraw_tail), 0.3)
        # The diagnostic coverage is not a second exclusion applied on top
        # of a reference that already excluded the spatial loss pixels.
        for coverage in (0.0, 100.0, None):
            adjusted = dataclasses.replace(bundle, scene_processing_loss_pct=coverage)
            plan = build_render_plan(adjusted, analysis, RENDER_MODE, "p3")
            self.assertEqual(hdr.tone.reliable_tail_ev, reliable_tail_ev(plan))
