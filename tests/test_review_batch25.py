...
(curve-table interpolation, the Oklab punch path and the pre-merged gamut fit
remain); the HDR parity gates stay at 2e-4 / 2e-5 (p99). Recorded here so the
next reader does not expect a bit-exact HDR fast path from this change.
"""
...
class HdrOutputStageIsFloat64(unittest.TestCase):
    def test_abi_v10_on_both_sides(self) -> None:
        ...
        self.assertEqual(NATIVE_ABI_VERSION, 10)
        header = (ROOT / "cpp" / "include" / "dngscan_fast" / "agx_core.h").read_text(encoding="utf-8")
        self.assertIn("NATIVE_ABI_VERSION = 10;", header)
    ...
    def test_kernel_reads_exact_float64_stages(self) -> None:
        ...
        self.assertIn("double rec2020_to_xyz_f64[9];", hdr_h)
        self.assertIn("double xyz_to_output_f64[9];", hdr_h)
        self.assertNotIn("matching the\n  // NumPy", hdr_h)
        kernel = (ROOT / "cpp" / "src" / "hdr_core.cpp").read_text(encoding="utf-8")
        self.assertIn("mat3_exact_f64(plan.rec2020_to_xyz_f64, mapped)", kernel)
        self.assertIn("mat3_exact_f64(plan.xyz_to_output_f64, xyz)", kernel)
        bindings = (ROOT / "cpp" / "src" / "bindings.cpp").read_text(encoding="utf-8")
        self.assertIn('copy_matrix9_f64(obj.attr("rec2020_to_xyz"), plan.rec2020_to_xyz_f64)', bindings)
        self.assertIn('copy_matrix9_f64(obj.attr("xyz_to_output"), plan.xyz_to_output_f64)', bindings)
    ...
    def test_built_extension_matches_when_present(self) -> None:
        ...
        self.assertEqual(int(ext.native_abi_version()), 10)
