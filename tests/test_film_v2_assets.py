...
class FilmV2AssetTests(unittest.TestCase):
    def test_schema_and_provenance_per_kind(self) -> None:
        """Every file in the modular family carries schema 8, its declared
        kind, sane domains and full-length source hashes (plan §7.1; 8 =
        route-D illuminant tiers joined the route-C field in the ABI)."""
        stocks = _stock_files()
        self.assertGreaterEqual(len(stocks), 25)
        for stock in stocks:
            with self.subTest(kind="stock", name=stock):
                z = _load(stock)
                self.assertEqual(int(z["schema"]), 8)
