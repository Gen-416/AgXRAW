...
2026-09-20 的非胶片效率实施与测量见 [管线完成记录](reports/performance/performance-pipeline-completion.zh-CN.md)。该批按各解码路径的既有生产实现验证新旧等价；本文中的长期冷路径并行、胶片与设备后端提案仍需各自的实现和发布门禁，不能从该批完成推断它们已经启用。
