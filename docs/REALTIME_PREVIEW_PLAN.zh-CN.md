...
- 浏览器预览固定使用 JPEG q95、4:4:4。导出质量和色度采样仍由导出档位独立控制。
...
| Portra 400：第一帧 | 274.07 ms | 279.26 ms | 含胶片感色与胶片曲线 |
...
已有磁盘代理时，普通计划约 60–75 ms；Portra 400 计划仍为 216.29 / 224.27 ms，其中 `scene_tone_metrics` 为 107.34 / 110.47 ms，tone-plan 内的第二次 scene sample 为 79.11 / 79.23 ms。两者对同一最多 80 万像素样本重复执行胶片感色。计划编译应先生成一次不可变 transformed sample，scene metrics 与 tone plan 共享；这项不改变颜色算法，只消除重复计算。
