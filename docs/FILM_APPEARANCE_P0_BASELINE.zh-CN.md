...
```
色相差 中位 4.4°   p95 17.0°
log2 chroma 比 中位 -0.052     （几乎没有饱和度差异）
...
|---|---|---|---|---|---|
| Ektar 100 | **−0.743** | −0.809 | −0.831 | −0.640 | −0.601 |
| Portra 400 | **−0.619** | −0.672 | −0.691 | −0.562 | −0.544 |
| Vision3 250D | +0.060 | +0.193 | +0.014 | +0.003 | +0.004 |
| Velvia 100 | +0.226 | +0.052 | +0.212 | +0.264 | +0.365 |

`−0.62` 是 2^−0.62 ≈ **0.65 倍**——Portra 在 full 下只有 observe 约三分之二的
Oklab chroma，Ektar 只有 0.60 倍。而 **Vision3 和 Velvia 没有这个现象**（分别 +0.06
和 +0.23）。

这条把「偏弱」定位到了具体家族：问题在 C-41 负片印相链，不是整条 full。任何全局
strength 都会把已经不弱的 Velvia 一起推过头。

按区域看（Portra）：叶绿 −0.81、洋红 −0.99、肤色 −0.65、青天 −0.43。洋红和叶绿掉得
最多，青天最少。
