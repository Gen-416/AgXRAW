...
保存；胶片的载体缺陷是颗粒、halation、扫描噪声、Dmax 天花板——默认不运送，由数字载体的
长处（HDR、验证、低噪声）取代。正如 Koch 不会往 DSD 流里加炒豆声。颗粒、halation、bloom
在 film v2 里作为可选拨盘存在（默认 0，见 FILM_OPTICS_V2_PLAN）：口味交给用户，不是校准数据，
也不改变"默认只运送签名"的立场。
...
马赛克。早期 `ec4f986` 使用亮度排名排除，后续加入校正损失百分比后暴露了根本问题：
暗部损失也会删掉最高光，且两种损失可能重复计算。当前改为在独立 LibRaw 参考自己的坐标
内合并空间损失、筛出可靠样本，再映射到 Apple 的场景单位；参考与主图保持相同的 WB/EV。
有参考时 rho 仍受 0.25 上限约束；成功但为空的参考不给 HDR，无法取得参考则只允许明确
标记的图像域估计（最多 1 EV，rho=0）。`Raw9HdrCouplingTests` 同时验证不能虚授余量和
不能误删有效高光，而不是仅验证结果足够保守。

**教训**：全局百分比不携带空间位置；保守也需要证明，不可把错误归零当成正确的安全边界。
...
## 案例 14：介质原生黑位——把观看环境从胶片标定里请出去

三件事曾被烘成一件：负片响应、相纸/幻灯的介质物理响应、观看环境。v2 把
IEC 61966-2-1 的 1% 参考房间杂光（反射相纸）和 0.5% 投影杂光（反转片）
烘进拟合目标——每卷胶片的"黑"于是都是房间的黑：Portra 黑位虚高到 1.5%
线性（sRGB c33 的灰雾），Velvia 只比中灰低 2.55 EV。v3 的契约是**标定只
描述介质**：相纸黑=相纸自身 Dmax,幻灯黑=幻灯 Dmax 过声明的环绕外观翻译,
...
拟合域上界 +8.5——那个"反转片硬肩的表达边界"其实是杂光垫高目标后的副作
用,解耦后全 25 卷 `fit.pinned` 为空、rms ≤0.061;②Kodachrome 琥珀 crossover
的暗部摆幅从 7 码值回到 16 码值——杂光一直在稀释数据手册的层间漂移。
不变组验收精确成立：10 卷电影系（链上杂光本就为零）参数逐位不动,已钉入
回归。附带修正 full 模式输入域（LUT 观察者逆拟合于普通 Rec.2020,运行时
prefeed 曾使胶片观察者被近似施加两次——full 现跳过 scene-transform）与
胶片预设下的逐图自动 punch/view-brightness（编辑性补偿不属于标定,预设
归零/归一）。教训：**"标定对象"与"运行时组合"要用同一份契约书写**——
数学都对,组合错了,成片就不对。

## 案例 13：G/B 立案的溶解——先统一度量域,再谈形状

DiVERE 交叉验证挂了三轮的两项未决（"G 深阴影背离 rms 0.2"与"B 密度尺度
0.68–0.84"）,在第五轮一次投影后全部消失:rms R 0.092–0.100 / G
0.039–0.043 / B 0.077–0.103,拟合尺度 1.02–1.10。改动只有一个——把比较搬进
ISO 5-3 密度计量域（链 A 投影 Status A,链 B 输入投影 Status M+Cineon 矩
阵）。此前的链 A 供的是**逐染料配量**,而 DiVERE 曲线的两端都是**密度计
读数**:染料间串扰（品红染料在蓝带的吸收等）在读数里、不在配量里,于是被
错记进"形状分歧"。柯达 H-1-2383 与 5207 出版曲线三方对账后,输出侧 gamma
（≈4）、负片侧 gamma（0.50–0.56/logE）双双吻合,残余 ~1.1× G 尺度归因于
Cineon 通用矩阵与卷特定光谱关系之差。教训:**跨实现比较先问"两边的数字
在哪个仪器的度量域里",再谈残差**——三轮"未决"其实一开始就没有在同一个
域里比;以及标准矩阵（Cineon Status M→印相密度）是时代产物,卷特定的光谱
推导可以合法地偏离它。

## 案例 12：Velvia 的品紫地板——一次由用户观感触发的三层归因

用户的一句"datasheet 版会那么红是正确的吗？"揪出了链条最深处的一个数据
bug。表症：Velvia full/datasheet 深阴影渲染为 12:1 的品紫（绿通道近零）,
教程文字却写"蓝黑带一点品"。逐层归因：

1. **[数据加载,主因]** vendored 光谱 profile 的染料密度表只在有限波段有值
   （Kodachrome 仅 425–695nm）,域外 NaN;载入端 `nan_to_num(0)` 把 NaN 填成
   零密度＝**完全透明**。Dmax 组在紫外/红外端漏 ~70% 的光进 CMF 的蓝、红
   响应瓣——R/B 通道被抬 4–10 倍而 G 保持诚实,合成品紫。底座声明（染料/
   基底域外**边缘保持**）在阶段一就写进了 spectral_base 的 docstring,但从
   未在载入端执行;上游 spektrafilm 的等价语义（`density_to_light` 把 NaN
   透射置零＝域外不透光）证实零填充是对上游的反向误读。
2. **[观看物理,次因]** 反转片观看链声明为零 flare——负片印相端早已为同类
   "阴影生硬"问题加过 1% IEC flare,反转片被漏掉。真实投影/灯箱有 0.5–1.5%
   杂光,Dmax 残色物理上不可见。现加入声明的 0.5%（ISO 3664 投影条件级）。
3. **[外部裁决]** 富士 AF3-202E 数据手册 Status A 平台值 R 3.38/G 3.82/
   B 3.68：真实地板是**温和**的红倾品红（G−R 密度差 0.44≈2.8:1）,方向与
   教程原文一致,幅度被泄漏放大了一个数量级。

修正后四卷反转片 Dmax 地板回到 ±6% 中性;Velvia 中性场蓝通道 −2 EV 处从
1.84 回落到 1.16。附带的连锁澄清：DiVERE 交叉验证中 B 密度尺度 0.68–0.76
的"偏离"随修正回升到 0.82–0.84——旧的 B 形状合流部分是幻影紫外印相通道
（负片 380–405nm 染料缺口零填充成透明,而 2383 层 0 的本征紫外敏峰恰在
385nm）的巧合产物。教训：**域外语义是光谱链的地基构件,"声明了"不等于
"执行了"——每条底座声明都应有直接钉死它的单测**;以及用户对成片的直觉
观感是有效的缺陷信号,值得完整归因而不是先入为主地辩护。
