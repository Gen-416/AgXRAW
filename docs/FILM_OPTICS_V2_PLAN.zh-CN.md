...
> 仍开放的一项（2026-09-02 审查 R-P3-5）：§P2 列出的 "RAW clip confidence/cap"
> 尚未接入 halation 源（`halation_component_source` 只读曝光，不读剪切置信），
> 剪切像素的光源亮度目前按其解码值参与散射。
>
> 范围：`full v2` 胶片显影路径中的空间成像。AgX、胶片特性曲线、B1/timing/
> 相纸显影/B2、SDR/HDR 交付均不在本计划中重写。
>
> **状态变更（2026-08-10，全量 review 之后）：P4/P5 暂停，空间光学与主管线分立。**
...
> **NN expand 账目闭合（2026-08-14）**：运行时早已满足本节合同（area_decimate
> + 双线性 upsample_rows，全链无 NN）。R1/R4 时代记录的"4.73 未决缺陷"经实测
> 判定为**度量伪影**：blockiness 图表(1152px)小于 spread 网格上限(1408)，回采
> 样根本没有运行；倾斜边缘唯一的过渡尖峰混叠在 8px 模位上（比值随任意探测模
> 数单调增长 0.42→6.92，自相关无周期）。度量已重写为真实合同检测：降采样
> 生效尺寸(1400×2100)+多行中位+排除边缘邻域+同时读 legacy 模数与真实双线性
> 结点间距——实测 0.78–1.03 全部平坦（NN 块会在结点间距上远超 1），p0 门改为
> 双边区间断言。P5 实施记录第④点的"反转门"与 R1 整改记录第④点的"重新开账"
> 表述以此为最终更正：散射污染是真的，但污染之下没有块，只有度量在测边缘
> 落点。
...
> **P5f：halation 细源门（2026-08-31 审查）**：残差回注的逐点扣除
> `A@U(E)` 在全分辨率求值，而 spread map 的门却开在降采样网格上。门是
> log 曝光的 smootherstep，`gate(decimate(E)) ≠ decimate(gate(E))`——小于
> 一个降采样格的光源（61 MP 全幅约 <25.6 µm）平均后跌破门槛，map 里不存
> 在。实测（+6.5 EV 1 px 白源，7× 降采样）：扣除能量 100% 消失、无任何
> 光晕，光源像素自身损失 7.7%/3.0% 的 R/G 层曝光（amount=1）；恒等网格
> （≤spread 上限的渲染）两侧平衡到 0.1%，即缺陷只在导出分辨率出现，还
> 违反 preview/export 一致性。这与批十六 bloom 的能量消失同类，教训当时
> 只用到了扣除侧。修正合同：**门升到全分辨率，扩散留在降采样网格**——
> pass A 的同一行带循环里按分量累积门后转移源 `A_i@U_i(E_full)` 再面积
> 降采样（照 bloom 细档模式；全零 transfer 的分量不付网格），恒等网格走
> 原路径逐字节不变（预览零开销，freeze 0 漂移）。bloom 交互：细源在
> pass A 流式时 glow map 尚不存在，故细源取未加辉光的场景，辉光贡献以降
> 采样网格上的非负修正项 `gate(bloomed proxy) − gate(raw proxy)` 闭合
> （glow 只加光、门后源对层曝光单调——但层曝光对场景 RGB 在压缩/色度场/
> 有符号观察者下并不单调，修正项可以为负，clamp 是有意的单侧近似而非
> 浮点守卫，2026-09-03 数学审查实测 R 层 −1.20）；残余近似为"仅靠
> glow 抬过门槛的亚格源仍按 proxy 分辨率开门"，二阶量。§11.1 的档位取舍
> 论证只覆盖了核的 Nyquist、没覆盖门的非线性，本记录补上缺口。门禁：
> `tests/test_halation_fine_source.py`（钉缺陷本身、能量闭合 ≤1%、行带
> 分割在整数网格比下逐字节确定、非整数比只到 float32 累加舍入 ~1e-7、
> 恒等网格字节不变、降采样下行带=全帧 oracle、bloom 修正非负）；批十三 RSS 镜像脚本与 `tools/film_optics_report.py` 的
> pass A 镜像同步加入细源调用。
>
> **2026-09-03 数学审查（P1）**：give 侧曾在乳剂散射（§5.1 mix）之后的
> 层曝光上开门，而 take 侧的细源来自散射前的场景——两侧不是同一量。
> 预览间距散射为恒等所以平衡数字成立；导出间距（36 mm/6016 px，halo 4 px）
> 1 px 源经 mix 峰值只剩 29–53%，give 门开得少，take 比 give 多约 3.5×，
> 残差形式凭空造能。修正：give 用散射前曝光（`halation_reinject_rows`
> 的 `give_lin`），两侧同量；§5.2 把源定义在 E^s 上的写法两条路径都
> 没实现，以代码为准（源与 DC 项都在散射前）。
>
> 二次审查整改（同日）：① **性能**——全像素跑 Stage A 实测 ~18 s/61MP
> 帧（层曝光 10.3 s + 门/降采样 7.3 s），改为行级候选掩码：利用可证明的
> 上界（压缩后任一通道 ≤ 压缩前最大通道；有符号观察积 ≤ 正部行和 ×
> max 通道；色度场 ≤ 2^max(0,ΔLUT)）预筛，低于最低门槛的整行跳过——
> smootherstep 在 t0 以下精确为 0，跳过逐字节等价；无门内容的帧只付
> 每像素一次 max+比较。② `_OPTICS_FIXED_MIB` +68（细源累积器在 2048
> 网格 ~67 MiB，两个活跃分量）。③ finish_maps 校验 begin 缓存的
> prep 与实参一致（stock 按 _load_v2 缓存身份比较），防两套曝光坐标
> 混入一张 map。④ oracle 路径补 hal_fine/hal_prep 释放（对齐
> bloom_fine）。⑤ **预览的信息边界，记档不改**：预览场景本身是降采样
> 代理，亚像素高光在进入管线前已被均值稀释——预览天然显示不出亚像素
> halo，导出才有；这是预览的固有边界（与颗粒纹理随采样率变化同类），
> 不是本合同的违约。bloom bracket 对"仅靠 glow 抬过门槛的亚格源"的
> 二阶残差（审查测 ~0.14% 能量）维持已记档的接受。
