...
JPEG 自动质量范围固定为 **95–99**，100 保留为显式 archive 或手动设置。SDR 与 HDR JPEG 都以 q99 / 4:2:2 为参考，参考未通过回读时向下寻找最高质量的有效参考。x265 HEIF 使用独立刻度：以 q95 / 4:4:4 / 默认 10-bit 为参考，试 95、92、90、85、80、70，再在合格质量及更高一档试 4:2:2、4:2:0。HEIF 的声明参考不合格时导出失败，不以下降参考质量来放宽预算。所有路径在失败时保留已有文件。

JPEG 在通过回读的候选中，至少节省 5% 字节才采用更小文件。HEIF 在满足同样误差预算后按实际总字节数选择，避免主图的小幅收益因大 gain map 稀释而无法参与后续联合优化。默认亮度预算为不超过 1.0 码值 RMSE、8×8 局部绝对误差 p99 不超过 1.5 码值；当参考编码自身超过这两个值时，允许相对参考有限增加。色度 RMSE 限制为参考 ×1.10+0.10 码值。SDR JPEG 另试最终质量的 4:2:0，只有仍满足参考的亮度／色度预算、且相对选中的 4:2:2 再节省至少 5% 才采用。所有公式见 `dngscan/auto_encode.py`。
...
## 2026-09-19：独立辅助图编码与 SDR HEIF

输出格式现为 `sdr`、`sdr-heic`、`ultrahdr`、`ultrahdr-heic`。SDR HEIF 直接编码 SDR 成片，不进入 HDR 预算或 gain-map 生成，因此 HDR 计划为 0 EV 也能导出 HEIF。x265 支持 sRGB/P3、8/10-bit、420/422/444；当前 SDR master 仍为 8-bit，10-bit 编码不会恢复之前已经量化掉的信息。逐文件检查 ICC、尺寸、位深、采样与解码误差，拍摄元数据复制前后核对压缩内容签名，最后原子替换目标文件。Apple 的 SDR writer 在本机实测只可靠兑现 8-bit/420，其他组合明确拒绝，自动搜索需要 libheif/x265。

仅把 ImageIO gain-map quality 从 95 提到 97、98、99，未能修复本轮 24 MP 样本的局部 HDR 损失：四档最大局部亮度误差均约 1.55，仍被原门禁拒绝。现在自动 HDR HEIF 先生成一次高精度 RGB 辅助图，将其作为数值图读取，再由 x265 独立编码为 8-bit/444，搜索 q95、90、85、80。每个候选都从同一份初始辅助图编码，不连续叠加重压。ISO tmap 参数、辅助图尺寸及其他图像 payload 保持不变；不为数值 gain map 附加显示 ICC。遇到不支持的辅助图格式则保留验证过的参考。辅助图没有缩小分辨率；不把系统可能忽略的下采样请求算作优化成果。
