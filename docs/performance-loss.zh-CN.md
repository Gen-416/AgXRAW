...
最终 RAW／证据专项两种模式各运行 182 项：NumPy 166 通过、16 跳过，严格 Rust 181 通过、1 跳过。共同跳过的是缺少固定路径样片的旧 reference-load 测试；NumPy 额外跳过 15 项要求启用 native 的既有测试。本批新增的 34 项在两种模式下均通过。
