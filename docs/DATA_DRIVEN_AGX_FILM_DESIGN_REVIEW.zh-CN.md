...
> **处置记录（第四轮审查，2026-08-27）**——审查正文附于文末 §14。
> - **F1 Route D 统计字段语义（P2）→ 已修。** `stocks_adopting` 改名 `responses_adopting`（两份记录同步，
>   路线 C 记录的预设计数改名 `presets_adopting`），`adoption_rule` 与 `FAMILY_THRESHOLD` 注释明确
>   以唯一光谱响应为计数单位。
> - **F2 固定候选门是假门禁（P2）→ 已修。** 决策抽成纯函数 `decide_tier`；记录保存逐 seed 摘要；门
>   （i）用构造的摘要让逐 seed 混选与固定候选给出不同频率并断言固定候选结果，（ii）对记录中每个
>   (stock, 光源) 从逐 seed 摘要重算并与存储频率相等，且要求记录里至少一对混选频率不同——回退到混选
>   无法通过。
> - **F3 白轴插值缝（P3）→ 已修。** 烘焙时沿权重场扣除白点处 δ 的双线性采样残差（δ' = δ − w·c），
>   白色度采样严格为零，边界零区不受影响；运行时门收紧到 1e-9。
> - **F4 3×3 报告测试用旧字段（P3）→ 已修。** 注入 stock 清除 `chroma_delta_lut`，并断言调度结果与
>   observer 路径逐位相等、与场路径不同。
>
...
## 14. 第四轮审查原文（2026-08-27，独立复算，未改文件）

1. **[P2] Route D 统计字段语义错误。** `stocks_adopting` 当前实际统计的是去重后的 spectral responses，而 `adoption_rule` 仍写 `stocks`。当前结论不受影响，但 JSON 使用者可能套错分母或阈值。建议明确拆成 `responses_adopting` 与 `presets_adopting`，并声明 `FAMILY_THRESHOLD` 以 response 为单位。
2. **[P2] 固定候选回归测试仍是假门禁。** 测试没有调用 `stock_record()`，只根据已保存的 `tier_model` 重算前三个 seed，并允许 0.34 的误差。即使生产代码恢复成逐 seed 混选，这项测试仍可能通过。当前实现本身正确（独立重跑 C200/CIE A 全部 30 个 seed，准确复现固定 3×3、0/30）。
3. **[P3] 白轴"精确锚定"还存在极小插值缝。** 连续模型在白点为零，但 LUT 的四点双线性插值不保证白点采样也严格为零；精确中性像素又直接跳过 correction。趋近中性轴时存在最高约 3.47e-5 stop 的极小不连续。
4. **[P3] 3×3 报告测试仍写旧字段。** 测试设置的是 `chroma_lut=None`，实际分派字段已是 `chroma_delta_lut`，所以渲染仍走 field；报告字符串因手动修改 `stage_a_model` 而继续通过。

之前六项实质问题均已修复：边界跳变消失、provenance 完整、Route D 固定候选正确、矩阵诊断跟随真实 WB ladder、重复响应已去重、ridge sensitivity 已测生产 LUT。验证：1255 项测试通过，35 项条件跳过；旧最坏边界探针已降至约 0.00008/255 和 0.00023/255。本机未构建 `_native`，严格原生内核路径未本地复跑。工作区干净，main 与 origin/main 均为 d998575。
