...
## 路线 D 阶段 1:光源假设的代价(已测量)

工具 `tools/fit_illuminant_tiers.py`,记录
[illuminant_tier_cv.json](illuminant_tier_cv.json)(同折同锚纪律;训练白板
与 Bradford CAT 均在同光源下,镜像"已白平衡的运行时场景")。全 20 支底片
held-out p99 中位:

| 光源 | D55 假设误差 | 专档上限 | 可收回 |
|---|---|---|---|
| CIE A(钨丝 2856K) | 2.25 | 0.75 | **1.49** |
| LED-B3(高显色 LED ~4000K) | 1.37 | 0.68 | 0.73 |
| LED-RGB1(窄带三基色,压力档) | 4.09 | 0.75 | 3.30 |

两个结论:**(1)** 场族的表达力在所有测试光源下都够(专档 p99 一致落在
0.68–0.75,与 D55 同级)——代价完全在"猜错光源",不在模型;**(2)** 因此
阶段 2 是"假设管理"问题:WB/CCT 足以区分钨丝与日光(插值 D55↔A 是合理的
**光源假设**,必须如此称呼,不得称测量);~4000K 区间(磷光 LED/荧光/混光)
与窄带光源从 CCT 不可辨识,是 profile confidence 降级的真正依据——窄带
专档虽然可行,但运行时无法知道该用它。

阶段 2(未做):A 与 LED-B3 专档资产 + 按声明 WB 的档间插值 + 报告中的
光源假设声明与 confidence 降级。
