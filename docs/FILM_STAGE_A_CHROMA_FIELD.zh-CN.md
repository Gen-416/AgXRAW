...
状态:**阶段 1(测量)已完成,运行时未改动**。本文是路线 C(两路线纲领,
2026-08-26)的决策记录:Stage A 是否值得从固定 3×3 observer 升级为保持
曝光齐次性的色度场。
...
Owner 判据(2026-08-26):held-out p95/p99 显著下降才采纳。**已满足**。
阶段 2(运行时接入)是独立决策,须解决:

1. **域外守卫**:三次多项式在训练色度凸包外外推危险;运行时必须投影回
   凸包或在域外连续回退到 3×3。
2. 资产 schema 升级与 Stage A 求值路径切换(full 模式冻结将声明式重钉)。
3. 光源分档(D55/A/高显色 LED)是其后的扩展;按 WB 在档间插值必须称
   "光源假设",不得称测量。

per-stock 低压力例外(superia400/pro400h)在阶段 2 应保留 3×3——按 CV
结果逐 stock 选型,选型记录进资产。
