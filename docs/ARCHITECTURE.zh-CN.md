...
最小二乘解给出（Velvia 100：black −4.10 / white +4.48 / contrast 3.05 /
toe power 3.25 / shoulder power 8.01；拟合残差见"预设库与出处"一节）。
...
相对化处理，预设声明的相纸 Dmax 地板保留）：Velvia 曲线上 `toe_end_offset`
−2 移动 50.4% 的像素（平均 11.8 码值），而 `shoulder_start_offset` +2 只
移动 13.6%（平均 0.8 码值）——shoulder power 8.01 是预设库里最硬的肩，
合法曲线族留给"更柔"的空间本来就小，与上文"肩部自由度的几何"同构。

**放大机色头的光谱印相实现**：负片预设的 Y/M 滑杆不是后置 RGB 增益。响应
由该预设的拟合光谱印相模型推导——构建期把 CC 分色滤镜放进放大机光路重曝
相纸、按暗房惯例重解印相曝光时间（中灰亮度不变的约定，实测各档位全图中位
L\* 稳定在 45.2–45.9），运行时按 CC 档位插值。M 轴力道大于 Y 轴源于相纸
绿敏层的伽马更高（M +10CC 把全图中位 a\* 从 +0.8 推到 −21.3，而 Y +10CC
对 b\* 的移动约为其一半）。两档都在 0 时输出与不启用逐字节相同；反转片
物理上无印相环节，预设档案将其分类为无色头，CLI 传入直接报错。
...
Velvia 100 的实测比率场：蓝通道在 −2 EV 处 1.33、−4 EV 处 1.61，+1 EV 仅
1.04——漂移几乎全部住在中灰以下，量级未经外部 oracle 裁决。observe 模式
