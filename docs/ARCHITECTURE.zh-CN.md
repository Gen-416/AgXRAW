...
**比率场锚点（`--film-crossover`）**：**off**（默认，逐字节现状）——比率
增益按像素自身亮度归一，中性轴上的分区染色按构造抵消，是一条反隐藏白平衡
的安全轨，胶片性格保留在彩色内容上、中性灰严格中性；**datasheet**（实验）
——改为中灰锚定：0 EV 严格保持白平衡，中性轴两端按拟合器实测数据漂移。
Velvia 100 的实测中性场（v2，TH-KG3+CAT+Rec.2020 基底）：蓝通道在 −2 EV 处
1.84、−4 EV 处 3.16（近 4.0 护栏），+1 EV 仅 1.04——漂移几乎全部住在中灰
以下，量级未经外部 oracle 裁决。observe 模式
下 datasheet 与 off 逐字节相同：比率场整层不参与，开关自然惰性。
...
~0.68–0.76 仍偏离接触印相约定——两者已立案待查，未粉饰。
