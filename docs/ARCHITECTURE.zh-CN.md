...
b2，schema 9：每卷 Stage A 按 runtime-faithful、重复折的 held-out CV 选色度场或
3×3，携带报告用的 held-out 数字与 provenance 块（反射率库、CV 记录、拟合/烘焙
参数与库版本的哈希），全部 fail-closed 加载）。关闭 `--film` 即纯 AgX。逐通道滚降本身就是颜色操作——这就是"AgX 只管拉伸滚降、颜色全给胶片"
