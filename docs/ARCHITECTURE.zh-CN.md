...
边缘保持，反转片观看链并加入声明的 0.5% 投影 flare（ISO 3664 级；负片印
相端同类修正为 1% IEC flare）。修正后四卷反转片的 Dmax 地板全部回到 ±6%
内的中性黑，与富士数据手册 Status A 平台值（R 3.38/G 3.82/B 3.68，温和品
...
（边缘保持修正后重测）：B 密度尺度回升到 0.82–0.84（向接触印相约定收
敛——旧的 B 形状"合流"部分是幻影紫外印相通道的巧合产物：负片染料表
380–405nm 缺口被零填充成透明,而 2383 层 0 的本征紫外敏峰恰在 385nm,高
于其 695nm 红增感峰）,但三通道形状 rms 上行（R ~0.10、G ~0.20、B
~0.33）。当前解读：白光接触印相约定 vs DiVERE 逐通道曲线隐含的三色印相
机模型之争,叠加紫外/蓝敏层间串扰——需按评审指引把染料谱投影过 Status
A/M 密度计响应后重比,并以柯达原始 2383 特性曲线为第三方裁决。立案保持
开放，未粉饰。
