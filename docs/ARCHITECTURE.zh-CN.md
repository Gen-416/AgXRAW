...
dngscan 默认不做降噪，因此解拜耳也是主要的纹理选择。唯一的可选例外是仅色度数字化修复（`--chroma-nr`，默认 0）：一个频带受限、内容自适应（按每级 MAD 定阈）的算子，只去低频色斑、按构造不碰别的——见 `chroma_nr.py`。它是场景阶段的算子，SDR、AgX HDR formation 与 HDR pair 的两腿读的是同一份修复后的场景。DHT 适合低 ISO 的干净信号；重噪声
...
b2，schema 10：每卷 Stage A 按 runtime-faithful、重复折的 held-out CV 选色度场
（以解析 3×3 之上的修正 LUT 发布）或 3×3，携带报告用的 held-out 数字与 provenance
块（反射率库、CV 记录、拟合/烘焙参数、生成器源码与库版本的哈希），全部 fail-closed
加载）。关闭 `--film` 即纯 AgX。逐通道滚降本身就是颜色操作——这就是"AgX 只管拉伸滚降、颜色全给胶片"
...
风格作为分离的 look 层叠加**——look 不改 DRT，DRT 不含 look。2026-07-30 的观感
评估结论是 "observe 稳而淡、full 浓而坏"，缺的味道由 look 层补足，且只用两个
**已验证机制**做词汇：
