...
最小二乘解给出（Velvia 100（v2 光谱底座）：black −4.03 / contrast 3.03，
white 与 shoulder 顶在拟合域界 +8.5 / 10——反转片硬肩在这套曲线族里的
...
**放大机色头的光谱印相实现（v2）**：负片预设的 Y/M 滑杆不是平坦 RGB 乘数。
...
稳定），随包数据 `dngscan/data/color_head/*.npz`（float16，128 点 EV 轴对直接
光谱求解实测最大 0.016 档 < 0.02 验收门）；档位直接索引零 CC 插值，运行时
...
post-prefeed Rec.2020、逐通道 log₂ 曝光整形、四面体插值、域外钳制），开关
选择中性轴的供给方式：**off**（默认）——数字中性化变体，各通道除以中性阶
染色在该像素亮度曝光处的取值（存于同一 npz 的**有界** cast 曲线：色度校正
每通道限幅 [0.25, 4] 后亮度重归一；不烘第二只表——有界除数逐像素精确求
值时商的档位误差等于 datasheet 表自身，烘复合函数反而把 EV_Y 折点摆到网
格对角线上，实测最差 1.73 EV），介质灰阶偏中性两档以内严格中性，更深的
染色（如 Kodachrome 的洋红地板）如实保留为介质性格；
...
Velvia 100 的实测中性场（v2 数据，仍随预设发布）：蓝通道在 −2 EV 处 1.84、
−4 EV 处 3.16，+1 EV 仅 1.04——漂移几乎全部住在中灰以下，量级未经外部
oracle 裁决。observe 模式下 datasheet 与 off 逐字节相同：接管核心不参与，
...
当前拟合水平（v2 光谱底座：TH-KG3 印相光源、原生网格梯形积分、q 联合求解、
XYZ→CAT→Rec.2020 观看翻译）：25 支全部 rms ≤0.051——负片系 0.012–0.033、
反转片系 0.022–0.051、theatrical 引用系 0.035–0.047。新观看翻译下反转片与
电影系多项参数顶到拟合域界（velvia white@8.5、kodachrome black@−2.5、
cine 系 shoulder@1.2 等），`fit.pinned` 字段逐项如实记录——这是 C1 曲线族
对硬肩滑片的表达边界，不是数据缺陷。

### 外部验证

`tools/crosscheck_2383.py` 用独立实现（DiVERE 的 Kodak 2383 曲线，曲线域约定从其
源码逐行核对）交叉验证——这把外部尺子已用三次：第一轮定位了旧通道捷径的缺陷
...
裁决范围）。第三轮现状（五支 2383 卷）：R rms 0.056–0.067、B 0.049–0.080
合流；**G rms 0.11–0.15（max 0.27–0.41）是当前开放问题**，B 密度尺度
~0.68–0.76 仍偏离接触印相约定。已完成的定位（vision3250d）：G 误差集中在
深阴影/印片近 Dmax 区（EV<−3 段 rms 0.197，最坏点 EV −6.0；EV>1 段仅
0.045）——这是 G 层肩部/Dmax 形状之争，我方光谱合成与 DiVERE 发表曲线
各执一词，需第三方源（柯达原始 2383 特性曲线）裁决；B 的残差同样在深端
最大。立案保持开放，未粉饰。
