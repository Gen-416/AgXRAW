...
def read_dark(path: Path) -> tuple[dict, dict]:
    ...
    for r in rows:
        ...
        if greens_present and int(r["ColorIndex"]) not in GREEN_INDICES:
            continue
        iso = int(r["ISO"])
        d = per_iso.setdefault(iso, {"bl": [], "rn": [], "row": [], "col": [], "tot": []})
        ...
        var_t = var_diff / 2.0
        if adc_step > 0:
            var_t = max(var_t - (adc_step ** 2) / 12.0, 0.0)
        rn = math.sqrt(var_t)
        d["rn"].append(rn)
...
def build(set_dir: Path, meta: dict | None) -> dict:
    ...
    entry: dict = {
        ...
        "source": {
            ...
            "set": set_dir.name,
            "url": f"https://y-g-jiang.github.io/data/collect/{set_dir.name}/",
    ...
    entry["banding_log2iso"] = [
        [round(math.log2(iso), 4),
         round(d["row_var"] / max(d["total_var"], 1e-12), 4),
         round(d["col_var"] / max(d["total_var"], 1e-12), 4)]
    ...
    if rel and anchor is not None:
        ...
        if a_iso not in rel:
            ...
            # interpolation in log-log (offline precision policy)
            from scipy.interpolate import PchipInterpolator
            xs = np.log2(np.asarray(sorted(rel)))
            ys = np.log2(np.asarray([rel[i] for i in sorted(rel)]))
            rel_at_anchor = float(2.0 ** PchipInterpolator(xs, ys)(math.log2(a_iso)))
        else:
        ...
        entry["read_noise_log2iso_log2e"] = [
            [math.log2(i), math.log2(max(dark[i]["rn_dn"] * gain_curve[i], 1e-6))]
            for i in sorted(gain_curve) if i in dark]
        ...
        for k in range(2, len(u) - 1):
            flat_before = u[k - 1] / u[k - 2] < 1.08
            flat_after = u[k + 1] / u[k] < 1.08
            if flat_before and flat_after and u[k] / u[k - 1] > 1.15:
                jumps.append(isos[k])
